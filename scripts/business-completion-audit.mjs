...
const layout=requireAll('app/_layout.tsx',['assistant','qr-studio','notifications','intelligence','capabilities','enterprise']);
const home=requireAll('app/index.tsx',['Business CRUD','QR Studio','Custom Notifications','Kleenest AI','Advanced Intelligence','Capability Control Plane','optional Fleet handoff']);
...
for(const [name,source] of Object.entries({layout,home,locations,locationClaims,reviews,reviewEvidence,qr,comms,intel,prevention,governance,enterprise,enterprisePortfolio,capabilityUi,capabilityService,ai,tiers,intelligenceService,workspace})){
 if(source.includes('JSON.stringify(value??{},null,2)'))throw new Error(`${name} reintroduced raw JSON payload presentation`);
}
console.log('Business completion convergence audit passed with hardened Supabase-to-UI parity coverage.');
