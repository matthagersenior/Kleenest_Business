...
const auth=requireAll('app/auth.tsx',["router.replace('/')",'signInBusiness','await refresh()','Claim your business location for free','No subscription or payment is required']);
...
for(const [name,source] of Object.entries({home,enterpriseLocation,locations}))if(source.includes('asChild><Action')&&!source.includes('onPress?:()=>void'))throw new Error(`${name} contains Link-wrapped Action controls that do not forward navigation onPress`);
